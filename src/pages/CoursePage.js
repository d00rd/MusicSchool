...
import { enrollUserInCourse } from '../utils/enrollmentUtils';
  ...
  useEffect(() => { 
    async function fetchCourseData() {
      ...
      if (!courseId || typeof courseId !== 'string' || courseId.trim() === '') {
        setLoading(false);
        console.error("fetchCourseData aborted: Invalid courseId detected.");
        return;
      }
      if (!courseId) {
        setLoading(false);
  ...
  if (!courseId) {
    return (
      <div style={{ padding: "40px", textAlign: "center" }}>
        <h1>Error: Course ID Missing</h1>
        <p>Cannot load course details. Please ensure you clicked a valid course link.</p>
